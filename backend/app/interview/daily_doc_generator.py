...
from app.interview.learning_sources import format_source_links, handbooks_for_topic, github_files_for
...
- 主题：{task.topic}
- 阶段：{task.title}
- 章节：{task.section_title or task.title}
- 阶段目标：{task.goal}
...
重要：今日章节是「{task.section_title or task.title}」。面试题必须围绕该章节与上方知识点，不要写成别的 Prompt/幻觉/评测子题（除非章节本身就是那个主题）。
    ...
    *,
    ...
    for i, b in enumerate(bullets, 1):
        knowledge_parts.append(
            ...
            f"- **对照**：打开下方参考链接中与「{section}」相关的手册章节，用原文核对一遍。"
        )
