...
- [ ] 定义 `OfficialProvider` 枚举（仅用于 adapter_type='official'）
  - [ ] `deepseek`
  - [ ] `qwen`
  - [ ] `openai`
  - [ ] `anthropic`
...
- [ ] 定义 `SessionConfig` 模型
  ...
  - [ ] `model_config_id`: UUID, FK(model_configs.id), nullable (引用用户的模型配置)
  - [ ] `model_id`: String(100), nullable (内联配置，向后兼容)
  - [ ] `adapter_type`: String(20), nullable (内联配置)
  - [ ] `provider`: String(50), nullable (内联配置)
...
- [ ] 定义 `ModelConfig` 模型
  ...
  - [ ] `adapter_type`: String(20), indexed - 适配器类型 (official/openrouter/ollama/vllm)
  - [ ] `provider`: String(50), nullable, indexed - 供应商名称（仅 official 类型需要）
  - [ ] `model_id`: String(100) - 模型 ID
  - [ ] `base_url`: String(500), nullable - 自定义 Base URL
...
**字段说明：**
- `adapter_type`: 必填，值为 official/openrouter/ollama/vllm
- `provider`: 仅当 adapter_type='official' 时必填，值为 deepseek/qwen/openai 等
- `model_id`:
  - official: deepseek-chat, qwen-plus 等
  - openrouter: google/gemini-2.0-flash-001 等
  - ollama: llama3.2 等
  - vllm: Qwen/Qwen2.5-72B-Instruct 等
