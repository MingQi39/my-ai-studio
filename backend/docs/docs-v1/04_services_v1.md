...
**适配器架构对接说明：**
- 使用 Phase 3 定义的四类适配器：OFFICIAL, OPENROUTER, OLLAMA, VLLM
- 通过 `adapter_factory` 创建适配器实例
- 从 `providers.yaml` 读取配置，从环境变量读取 API Key
...
- [ ] 实现 `update_session_config(session_id: UUID, user_id: UUID, data: SessionConfigUpdate) -> SessionConfig`
  - [ ] 验证会话归属
  - [ ] 更新配置参数
  - [ ] 验证 adapter_type、provider 和 model_id 有效性
...
- [ ] 实现 `create_model_config(user_id: UUID, data: ModelConfigCreate) -> ModelConfig`
  - [ ] 加密 API Key
  - [ ] 验证 `adapter_type` 有效性（OFFICIAL, OPENROUTER, OLLAMA, VLLM）
  - [ ] 如果是 OFFICIAL 类型，验证 `provider` 有效性（deepseek, qwen 等）
  - [ ] 如果设为默认，取消其他默认配置
...
- [ ] 实现 `list_model_configs(user_id: UUID, adapter_type: AdapterType | None = None, provider: str | None = None) -> list[ModelConfig]`
  - [ ] 按 `adapter_type` 筛选（可选）
  - [ ] 按 `provider` 筛选（可选，仅对 OFFICIAL 类型有效）
  - [ ] 按创建时间排序
...
- [ ] 实现 `get_default_config(user_id: UUID, adapter_type: AdapterType | None = None) -> ModelConfig | None`
  - [ ] 获取用户默认配置
  - [ ] 可按适配器类型筛选
...
**重要变更：** 使用 `adapter_factory` 创建适配器

- [ ] 实现 `get_adapter(config_id: UUID, user_id: UUID) -> BaseLLMAdapter`
  - [ ] 获取配置并解密 API Key
  - [ ] 根据 `adapter_type` 选择创建方法：
    - `OFFICIAL`: 调用 `adapter_factory.create_official(provider, model_id, api_key=...)`
    - `OPENROUTER`: 调用 `adapter_factory.create_openrouter(model_id, api_key=...)`
    - `OLLAMA`: 调用 `adapter_factory.create_ollama(model_id, api_key=...)`
    - `VLLM`: 调用 `adapter_factory.create_vllm(model_id, api_key=...)`
  - [ ] 返回适配器实例
...
**示例代码：**
```python
from app.core import adapter_factory, AdapterType, OfficialProvider

async def get_adapter(self, config_id: UUID, user_id: UUID) -> BaseLLMAdapter:
    # 获取配置
    ...
    # 根据适配器类型创建
    if config.adapter_type == AdapterType.OFFICIAL:
        return adapter_factory.create_official(
            provider=config.provider,
            model_id=config.model_id,
            api_key=api_key,
            base_url=config.base_url,  # 可选
        )
    elif config.adapter_type == AdapterType.OPENROUTER:
        return adapter_factory.create_openrouter(
            model_id=config.model_id,
            api_key=api_key,
        )
    elif config.adapter_type == AdapterType.OLLAMA:
        return adapter_factory.create_ollama(
            model_id=config.model_id,
            base_url=config.base_url,
        )
    elif config.adapter_type == AdapterType.VLLM:
...
### 3.1 ModelConfig 数据模型调整建议

为了支持新的适配器架构，`ModelConfig` 模型需要包含以下字段：

| 字段 | 类型 | 描述 | 示例 |
|------|------|------|------|
| `adapter_type` | AdapterType | 适配器类型 | OFFICIAL, OPENROUTER, OLLAMA, VLLM |
| `provider` | str \| None | 供应商名称（仅 OFFICIAL 需要） | deepseek, qwen, openai |
| `model_id` | str | 模型 ID | deepseek-chat, google/gemini-2.0-flash-001 |
| `base_url` | str \| None | 自定义 Base URL（可选） | https://api.deepseek.com |
| `encrypted_api_key` | str | 加密的 API Key | ... |

### 3.2 适配器类型使用场景

| 适配器类型 | 使用场景 | provider 必需 | model_id 格式 |
|-----------|---------|--------------|--------------|
| OFFICIAL | 官方直连（DeepSeek、Qwen 等） | ✅ | deepseek-chat, qwen-plus |
| OPENROUTER | 通过 OpenRouter 访问多模型 | ❌ | google/gemini-2.0-flash-001 |
| OLLAMA | 本地 Ollama 部署 | ❌ | llama3.2, llava |
| VLLM | 本地 vLLM 部署 | ❌ | Qwen/Qwen2.5-72B-Instruct |
...
- [ ] ModelService: 适配器创建正常（支持四种类型）
- [ ] ModelService: 正确使用 `adapter_factory` 创建适配器
...
- [ ] 创建 `backend/tests/services/test_session_service.py`
- [ ] 创建 `backend/tests/services/test_model_service.py`
- [ ] 创建 `backend/tests/services/test_chat_service.py`
- [ ] 创建 `backend/tests/services/test_file_service.py`
- [ ] 创建 `backend/tests/services/test_batch_service.py`
- [ ] 测试适配器工厂集成
- [ ] 所有测试通过
- [ ] 测试覆盖率 > 80%
...
1. **适配器创建** - 必须使用 `adapter_factory`，不要直接实例化适配器类
2. **事务管理** - 多表操作必须使用事务
3. **权限验证** - 所有操作必须验证资源归属
4. **API Key 安全** - 永远不要在日志中输出 API Key
5. **文件安全** - 验证文件类型，防止恶意文件
6. **流式响应** - 正确处理连接中断
7. **错误传播** - 服务层异常应转换为业务异常
8. **性能考虑** - 大量消息历史需要分页加载
9. **适配器类型验证** - 创建配置时验证 adapter_type 和 provider 的组合有效性
...
```python
# 官方直连
adapter = adapter_factory.create_official(
    provider="deepseek",
...
# OpenRouter
adapter = adapter_factory.create_openrouter(
    model_id="google/gemini-2.0-flash-001",
    api_key=decrypted_key,
)

# Ollama（占位）
adapter = adapter_factory.create_ollama(
    model_id="llama3.2",
    base_url="http://localhost:11434",
)

# vLLM（占位）
adapter = adapter_factory.create_vllm(
...
```python
from app.core import LLMException, RateLimitError

try:
    async for chunk in await adapter.chat_completion(messages):
        yield chunk
    ...
    # 处理其他 LLM 异常
    logger.error(f"LLM error: {e.error_code}")
    raise
```
